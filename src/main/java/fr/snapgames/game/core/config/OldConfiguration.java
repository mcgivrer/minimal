 ...
 * <li>Boolean,</li>
 * <li>String.</li>
 * <li>{@link Vector2D} a new mathematical 2 dimensions vector used in the
 * {@link fr.snapgames.game.core.math.PhysicEngine} (mainly)</li>
 * <li>{@link fr.snapgames.game.core.math.Material} the physic engine required
 * material definition to make math processing.</li>
 * </ul>
 ...
 * @since 0.0.1
 * @deprecated preferred use of
 *             {@link fr.snapgames.game.core.configuration.Configuration},
...
public class OldConfiguration {
    ...
    public OldConfiguration(String file) {
        ...
        try {
            ...
            System.err.printf("OldConfiguration:The configuration file %s can not been loaded: %s%n", file,
                    e.getMessage());
        }
    }
     ...
     * @param defaultValue if no value exists in {@link OldConfiguration}, instead
    ...
    /**
     * Retrieve a value as Dimension from {@link OldConfiguration} from a string
     * format "dim([width],[height])".
     *
     * @param key          name of the configuration key to be loaded
     * @param defaultValue if no value exists in {@link OldConfiguration}, instead
     *                     use the defaultValue.
    ...
    public Dimension getDimension(String key, Dimension defaultValue) {
        ...
        if (!param.equals("") && param.startsWith("dim(") && param.endsWith(")")) {
            ...
            int width = Integer.parseInt(k[0]);
            int height = Integer.parseInt(k[1]);
            return new Dimension(width, height);
        } else {
            System.err.printf("OldConfiguration: Dimension value not found for %s, use %s as default value.%n", key,
                    defaultValue);
            return defaultValue;
        }
    }
    ...
    /**
     * Retrieve a value as Vector2D from {@link OldConfiguration} from a string
     * format "dim([width],[height])".
     *
     * @param key          name of the configuration key to be loaded
     * @param defaultValue if no value exists in {@link OldConfiguration}, instead
    ...
    public void save() {
        ...
        try (OutputStream output = new FileOutputStream(rootPath + filePath)) {
            ...
            System.err.printf("OldConfiguration:Unable to save configuration into properties file: %s%n",
                    e.getMessage());
        }
    }
